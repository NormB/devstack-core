...
#
#######################################
# FastAPI API-First Initialization Script with Vault Integration
#
# Description:
#   Initializes the API-first FastAPI implementation with Vault-managed configuration
...
#   API_FIRST_IP       - Service IP address (default: 172.20.0.31)
#   SERVICE_NAME       - Internal service identifier (fixed: api-first)
#   CERT_DIR           - Certificate storage path (fixed: /etc/ssl/certs/api-first)
#   VAULT_CERT_DIR     - Vault certificate source (default: ~/.config/vault/certs)
#   ENABLE_TLS         - TLS enablement flag (fetched from Vault)
#   API_FIRST_ENABLE_TLS - Exported TLS state for application
#
# Usage:
#   ./init.sh
...
#   # Set secret/api-first: {"tls_enabled": "false"} in Vault
#
#######################################
...
SERVICE_NAME="api-first"
SERVICE_IP="${API_FIRST_IP:-172.20.0.31}"
CERT_DIR="/app/certs"
...
#######################################
info() { echo -e "${BLUE}[API-First Init]${NC} $1"; }
...
#######################################
success() { echo -e "${GREEN}[API-First Init]${NC} $1"; }
...
#######################################
warn() { echo -e "${YELLOW}[API-First Init]${NC} $1"; }
...
#######################################
error() { echo -e "${RED}[API-First Init]${NC} $1"; exit 1; }
...
#   SERVICE_NAME - Service identifier for secret path (api-first)
...
#   API_FIRST_ENABLE_TLS - Modified: Exported TLS state for application
# Arguments:
#   None
# Returns:
#   0 - Always returns success (graceful degradation)
...
#   - Exports API_FIRST_ENABLE_TLS for consumption by start.sh
#######################################
setup_tls_certificates() {
    if [ "$ENABLE_TLS" != "true" ]; then
        info "TLS is disabled"
        export API_FIRST_ENABLE_TLS="false"
        return 0
    fi
    ...
    if [ -f "$vault_cert" ] && [ -f "$vault_key" ]; then
        ...
        export API_FIRST_ENABLE_TLS="true"
    else
...
# Coordinates the complete initialization sequence for the API-first FastAPI application
...
main() {
    echo ""
    echo "========================================="
    echo "  FastAPI API-First Initialization"
    echo "========================================="
    echo ""
