#!/bin/bash
################################################################################
# PostgreSQL AppRole Integration Test Suite
#
# Comprehensive test suite for validating PostgreSQL AppRole authentication
# migration. Tests that PostgreSQL successfully authenticates using AppRole
...
print_header() {
    echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}  PostgreSQL AppRole Integration Test Suite${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
    echo ""
}
...
test_container_running() {
    local test_name="PostgreSQL container is running"

    if docker ps --format '{{.Names}}' | /usr/bin/grep -q "dev-postgres"; then
        print_result "$test_name" "PASS" "Container dev-postgres is running"
    else
...
test_approle_entrypoint() {
    ...
    local entrypoint=$(docker inspect dev-postgres --format '{{json .Config.Entrypoint}}')
...
test_approle_mounted() {
    ...
    if docker exec dev-postgres test -f /vault-approles/postgres/role-id && \
       docker exec dev-postgres test -f /vault-approles/postgres/secret-id; then
...
test_no_root_token() {
    ...
    local env_vars=$(docker exec dev-postgres env)
...
test_approle_dir_set() {
    ...
    local approle_dir=$(docker exec dev-postgres printenv VAULT_APPROLE_DIR 2>/dev/null || echo "")
...
# Test 6: Check that PostgreSQL started successfully
test_postgres_started() {
    local test_name="PostgreSQL started successfully"

    if docker exec dev-postgres pg_isready -U postgres > /dev/null 2>&1; then
        print_result "$test_name" "PASS" "PostgreSQL is ready to accept connections"
    else
        print_result "$test_name" "FAIL" "PostgreSQL is not ready"
    fi
}
...
test_approle_auth_logs() {
    ...
    local logs=$(docker compose logs postgres 2>&1)
...
test_db_connection() {
    ...
    if docker compose exec -T postgres psql -U devuser -d devdb -c "SELECT 1" > /dev/null 2>&1; then
...
test_db_operations() {
    ...
    # Create test table
    if ! docker compose exec -T postgres psql -U devuser -d devdb -c \
        "CREATE TABLE IF NOT EXISTS approle_test (id SERIAL PRIMARY KEY, message TEXT);" > /dev/null 2>&1; then
    ...
    # Select test data
    local result=$(docker compose exec -T postgres psql -U devuser -d devdb -t -c \
        "SELECT message FROM approle_test WHERE message LIKE '%AppRole%';" 2>&1 | tr -d '[:space:]')
...
# Test 12: Verify policy enforcement (postgres cannot access mysql secrets)
test_policy_enforcement() {
    local test_name="Policy enforcement: postgres AppRole cannot access mysql secret"

    # Get postgres AppRole credentials
    local role_id=$(cat "${APPROLE_DIR}/postgres/role-id" 2>/dev/null || echo "")
    local secret_id=$(cat "${APPROLE_DIR}/postgres/secret-id" 2>/dev/null || echo "")

    if [ -z "$role_id" ] || [ -z "$secret_id" ]; then
        print_result "$test_name" "FAIL" "Cannot read postgres AppRole credentials"
        return
    fi

    # Authenticate with postgres AppRole
    ...
    if [ -z "$token" ]; then
        print_result "$test_name" "FAIL" "Failed to authenticate with postgres AppRole"
        return
    fi
    ...
    if docker exec -e VAULT_ADDR=http://vault:8200 -e VAULT_TOKEN="$token" dev-vault \
        ...
        print_result "$test_name" "PASS" "Least-privilege enforced: postgres cannot access mysql"
    fi
}
...
test_init_script_exists() {
    ...
    if [ ! -f "${PROJECT_ROOT}/configs/postgres/scripts/init-approle.sh" ]; then
        print_result "$test_name" "FAIL" "Script not found"
        return
    fi

    if [ ! -x "${PROJECT_ROOT}/configs/postgres/scripts/init-approle.sh" ]; then
...
test_docker_compose_config() {
    ...
    # Check entrypoint
    if ! /usr/bin/grep -A 5 "postgres:" "$compose_file" | /usr/bin/grep -q "init-approle.sh"; then
        echo "  ${RED}→${NC} Entrypoint not set to init-approle.sh"
        failures=$((failures + 1))
    fi

    # Check AppRole volume mount
    if ! /usr/bin/grep -A 30 "postgres:" "$compose_file" | /usr/bin/grep -q "vault/approles/postgres"; then
        echo "  ${RED}→${NC} AppRole credentials not mounted"
        failures=$((failures + 1))
    fi

    # Check VAULT_APPROLE_DIR env var
    if ! /usr/bin/grep -A 20 "postgres:" "$compose_file" | /usr/bin/grep -q "VAULT_APPROLE_DIR"; then
